...
def train_generate(dataset, batch_size, few, symbol2id, ent2id, e1rel_e2):
    ...
    while True:
        if rel_idx % num_tasks == 0:
            random.shuffle(task_pool) # shuffle tasks
        task_choice = task_pool[rel_idx % num_tasks]  # choose task
        rel_idx += 1
        ...
        query_triples = query_triples + [[triple[2], triple[1], triple[0]] for triple in query_triples]
