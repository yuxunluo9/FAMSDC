...
class Trainer(object):
    def __init__(self, arg):
        ...
        # loop through all the arguments and assign them to self
        ...
        self.Matcher = Matcher(arg, self.embed_dim, self.num_symbols,
        ...
        self.rel2candidates = json.load(open(self.dataset + '/rel2candidates.json'))  #补充这个文件

        # load answer dict
        self.e1rel_e2 = defaultdict(list)
        self.e1rel_e2 = json.load(open(self.dataset + '/e1rel_e2.json')) #补充这个文件

    def load_symbol2id(self):
        symbol_id = {}
        rel2id = json.load(open(self.dataset + '/relation2ids'))  #包括测试任务重的id吗？
    ...
    def load_embed(self):
        ...
        if self.embed_model in ['DistMult', 'TransE', 'ComplEx', 'RESCAL']:
            ent_embed = np.loadtxt(self.dataset + '/embeddings/{:d}/entity2vec.'.format(args.embed_dim) + self.embed_model)
            rel_embed = np.loadtxt(self.dataset + '/embeddings/{:d}/relation2vec.'.format(args.embed_dim) + self.embed_model)  # contain inverse edge
    ...
    def build_connection(self, max_=100):
        ...
        with open(self.dataset + '/path_graph') as f:  # what is path_graph?
            lines = f.readlines()
            for line in tqdm(lines):
                e1, rel, e2 = line.rstrip().split()
                self.e1_rele2[e1].append((self.symbol2id[rel], self.symbol2id[e2]))  # 1-n  # symbol2id是怎么编辑的
                self.e1_rele2[e2].append((self.symbol2id[rel], self.symbol2id[e1]))  # n-1 #双向边
    ...
    def eval(self, mode='dev', meta=False):
        ...
        for query_ in test_tasks.keys():
            ...
            support_triples = support_triples + [[triple[2], triple[1], triple[0]] for triple in support_triples]
            ...
            test_tasks_triplets = test_tasks[query_][few:] + [[triple[2], triple[1], triple[0]] for triple in test_tasks[query_][few:]]

            for triple in test_tasks_triplets:
                true = triple[2]
                query_pairs = []
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler('./logs_/{}.txt'.format(args.prefix), mode='w')
    ...
    if args.test:
        ...
        trainer.eval_()  ## further think it
        trainer.test_()
